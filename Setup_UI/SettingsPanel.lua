...
hooksecurefunc(SettingsCategoryListButtonMixin, 'Init', function(self, initializer)--列表 Blizzard_CategoryList.lua
    ...
    if category then
        WoWTools_ChineseMixin:Set_Label_Text(self.Label, category:GetName())
    end
end)
hooksecurefunc(SettingsCategoryListHeaderMixin, 'Init', function(self, initializer)
    if not InCombatLockdown() then
        WoWTools_ChineseMixin:Set_Label_Text(self.Label, initializer.data.label)
    end
end)
...
hooksecurefunc(SettingsPanel.Container.SettingsList.ScrollBox, 'Update', function(frame)
    ...
    WoWTools_ChineseMixin:Set_Label_Text(SettingsPanel.Container.SettingsList.Header.Title)--标提
    for _, btn in pairs(frame:GetFrames() or {}) do
        local lable
        if btn.Button then--按钮
            lable= btn.Button.Text or btn.Button
            if lable then
                WoWTools_ChineseMixin:Set_Label_Text(lable)
            end
        end
        ...
        elseif btn.Text and btn.data and btn.data.name and btn.data.name then
            WoWTools_ChineseMixin:Set_Label_Text(btn.Text, btn.data.name)
        end
        if btn.SubTextContainer then
            WoWTools_ChineseMixin:Set_Label_Text(btn.SubTextContainer.SubText)
        end
    end
end)
hooksecurefunc('BindingButtonTemplate_SetupBindingButton', function(_, button)--BindingUtil.lua
    ...
    if text==GRAY_FONT_COLOR:WrapTextInColorCode(NOT_BOUND) then
        ...
        WoWTools_ChineseMixin:Set_Label_Text(button, text)
    end
end)

hooksecurefunc(KeyBindingFrameBindingTemplateMixin, 'Init', function(self)
    if not InCombatLockdown() then
        WoWTools_ChineseMixin:Set_Label_Text(self.Label)
    end
end)
...
local function InitControlDropdown(containerFrame, _, name, tooltip)
    ...
    WoWTools_ChineseMixin:Set_Label_Text(containerFrame.Text, name)
...
local function InitControlSlider(containerFrame, _, name, tooltip)
    ...
    if raid then
		WoWTools_ChineseMixin:Set_Label_Text(self.GraphicsQuality.Text, SETTINGS_RAID_GRAPHICS_QUALITY)
	else
        WoWTools_ChineseMixin:Set_Label_Text(self.GraphicsQuality.Text, BASE_GRAPHICS_QUALITY)
	end
...
hooksecurefunc(SettingsAdvancedQualityControlsMixin, 'Init', function(self, settings, raid, cbrHandles)
    ...
    WoWTools_ChineseMixin:Set_Label_Text(self.ShadowQuality, SHADOW_QUALITY)
	WoWTools_ChineseMixin:Set_Label_Text(self.LiquidDetail, LIQUID_DETAIL)
	WoWTools_ChineseMixin:Set_Label_Text(self.ParticleDensity, PARTICLE_DENSITY)
	WoWTools_ChineseMixin:Set_Label_Text(self.SSAO, SSAO_LABEL)
	WoWTools_ChineseMixin:Set_Label_Text(self.DepthEffects, DEPTH_EFFECTS)
	WoWTools_ChineseMixin:Set_Label_Text(self.ComputeEffects, COMPUTE_EFFECTS)
	WoWTools_ChineseMixin:Set_Label_Text(self.OutlineMode, OUTLINE_MODE)
	WoWTools_ChineseMixin:Set_Label_Text(self.TextureResolution, TEXTURE_DETAIL)
	WoWTools_ChineseMixin:Set_Label_Text(self.SpellDensity, SPELL_DENSITY)
	WoWTools_ChineseMixin:Set_Label_Text(self.ProjectedTextures, PROJECTED_TEXTURES)
	WoWTools_ChineseMixin:Set_Label_Text(self.ViewDistance, FARCLIP)
	WoWTools_ChineseMixin:Set_Label_Text(self.EnvironmentDetail, ENVIRONMENT_DETAIL)
	WoWTools_ChineseMixin:Set_Label_Text(self.GroundClutter, GROUND_CLUTTER)
